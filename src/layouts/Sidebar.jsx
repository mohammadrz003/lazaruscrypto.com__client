...
const Sidebar = ({ isSidebarVisible, onChangeSidebarStatus }) => {
  ...
  return (
    <>
      ...
      >
        ...
        <ul className="w-full flex flex-col mb-5 space-y-2 lg:hidden">
          {MENU_ITEMS.map((item) => {
            return (
              <li className="w-full">
                <Link className="w-full block py-2" to={item.link}>{item.name}</Link>
              </li>
            );
          })}
        </ul>
        <div className="flex space-x-3 lg:space-x-0 lg:flex-col lg:space-y-3">
